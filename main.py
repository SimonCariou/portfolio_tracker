...
class PortfolioCalculator():
    ...
    def get_prices(self, list_assets):
        ...
        for asset, url in dict_urls.items():
            ...
            price_tags = scraped_page.find_all('div', class_='priceValue___11gHJ')
            # prices[asset] = float(price_tags[0].text.split()[1].replace(",","")) #in case there are more than one, pick the first one
            prices[asset] = float(price_tags[0].text.split("$")[1].replace(",",""))
...
if __name__ == "__main__":
    ...
    total_portfolio_usd = calculator.calculate_global_portfolio_value(portfolio_value)
    total_portfolio_eur = calculator.convert(total_portfolio_usd, config.DEFAULT_FIAT, 'EUR')
    percent_change_portfolio_global = total_portfolio_eur/config.INITIAL_INVESTMENT_EUR - 1
    plus_value_after_taxes = calculator.calculate_plus_value_after_taxes(total_portfolio_eur)
    total_portfolio_after_taxes = config.INITIAL_INVESTMENT_EUR + plus_value_after_taxes
    percent_change_portfolio_after_taxes = total_portfolio_after_taxes/config.INITIAL_INVESTMENT_EUR - 1
